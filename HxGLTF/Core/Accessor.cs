...
#if DEBUG
        sw.Stop();
        Console.WriteLine($"[ReadData] {accessor.Name ?? "(unnamed)"} | Type: {accessor.Type.Id} | Comp: {accessor.ComponentType.Id} | " +
                          $"Stride: {(accessor.BufferView.ByteStride != 0 ? accessor.BufferView.ByteStride : 0)} | Count: {accessor.Count} | " +
                          $"TotalFloats: {data.Length} | Time: {sw.Elapsed.TotalMilliseconds:F2} ms");
#endif
